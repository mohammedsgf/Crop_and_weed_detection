            ...
            if predicted_class == 1:
                ...
                if predect > confi_thresh:
                    ...
                    #rescale the x,y,w and h to the image scale
                        ...
                        #update our list of bounding box coordinates, confidences,
                        #and class IDs
                        unkown_boxes.append([x, y, int(width), int(height),centerX ,centerY])
                        unkown_confidances.append(float(predect))
                        unkown_classIDs.append(predicted_class)

    #apply non-maxima suppression to suppress weak, overlapping bounding
    #boxes
    true_boxes = cv2.dnn.NMSBoxes(boxes, confidances, confi_thresh, NMSthresh)
    unkown_objects = cv2.dnn.NMSBoxes(unkown_boxes, unkown_confidances, 0, NMSthresh)
...
if __name__ == "__main__":
    ...
    frame = cv2.imread('C:\\Users\\MohammedSGF\\Desktop\\Senior Project\\WeedDetec\\Crop_and_weed_detection\\performing_detection\\data\\images\\test_6.jpg')
